...
struct VectorStep {
    ...
    int capacity;
    QString desc;
    QStringList codeLines; // 当前步骤的伪代码行
    int activeLine;        // 高亮行索引
};

// 派生自 VisualBase，用于可视化特定 STL 算法或数据结构的类
class VisualVector : public VisualBase {
    Q_OBJECT
public:
    // 构造函数，初始化 UI 界面布局
    explicit VisualVector(QWidget *parent = nullptr);
    // 执行动画的一步进操作
...
private:
    ...
    class CanvasWidget : public QWidget {
        VisualVector* p;
    public:
        CanvasWidget(VisualVector* parent) : QWidget(parent), p(parent) {}
    protected:
    ...
    std::vector<VectorStep> animSteps;
    int currentStep;
